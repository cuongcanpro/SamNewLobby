...
def list_json_file_in_folder(folder, root_folder):
    for name in os.listdir(folder):
        file_path = os.path.join(folder, name)
        if os.path.isfile(file_path):
            relative_path = os.path.relpath(file_path, root_folder)
            ...
            if (file_ext=='.json'):
                ...
                item.path = relative_path.replace('\\', '/')
                item.md5 = hashlib.md5(open(file_path, 'rb').read()).hexdigest()
                g_list_json_assets.append(item)
        else:
            list_json_file_in_folder(file_path, root_folder)

def export_list_file_for_web():
    global g_list_assets
    g_list_assets = []
    list_file_in_folder('../res', '../')
    global g_list_json_assets
    g_list_json_assets = []
    list_json_file_in_folder('../res', '../')
    list_assets = []
    list_json = []
    list_assets_md5 = {}
    ...
    data = 'var g_resources = ' + json.dumps(list_assets) + ';'
    data = data + '\n var g_hash_resources = ' + json.dumps(list_assets_md5) + ';'
