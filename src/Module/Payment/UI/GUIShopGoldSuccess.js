...
var GUIShopGoldSuccess = BaseLayer.extend({
    ...
    onButtonRelease: function (btn, id) {
        ...
        for (var i = 0; i < this.arrayBonusData.length; i++) {
            ...
            var pEnd = dispatcherMgr.dispatchEvent(UserMgr.EVENT_GET_POS_COMPONENT, this.arrayBonusData[i].type);
            if (!pEnd) pEnd = cc.p(0, 0);
            var addBonus = this.arrayBonusData[i].getSum();
            if (this.arrayBonusData[i].type == UserMgr.TYPE_DATA_GOLD) {
                var timeRun = effectMgr.flyCoinEffect(sceneMgr.layerSystem, addBonus, addBonus / 20, pStart, pEnd);
                var startGold = dispatcherMgr.dispatchEvent(UserMgr.EVENT_GET_VALUE, this.arrayBonusData[i].type);
                if (!startGold) startGold = 0;
                endGold = startGold + Number.parseFloat(addBonus);
                if (endGold <= userMgr.getGold()) {
                    var dataEffect = {};
                    dataEffect.type = this.arrayBonusData[i].type;
                    dataEffect.timeDelay = timeRun - 0.5;
                    dataEffect.end = endGold;
                    dispatcherMgr.dispatchEvent(UserMgr.EVENT_EFFECT_LABEL, dataEffect);
                }
                delay = delay + 0.5;
            }
            else {
                ...
                delay = delay + 0.5;
                if (this.arrayBonusData[i].type == UserMgr.TYPE_HOUR_VIP) {
                    ...
                    var startG = dispatcherMgr.dispatchEvent(UserMgr.EVENT_GET_VALUE, this.arrayBonusData[i].type);
                    var endG = startG + Number.parseFloat(addBonus);
                    if (endG <= userMgr.getCoin()) {
                        var dataEffect = {};
                        dataEffect.type = this.arrayBonusData[i].type;
                        dataEffect.timeDelay = delay + 0.5;
                        dataEffect.end = endG;
                        dispatcherMgr.dispatchEvent(UserMgr.EVENT_EFFECT_LABEL, dataEffect);
                    }
                }
                else if (this.arrayBonusData[i].type == ShopSuccessData.TYPE_DIAMOND) {
                    var startDiamond = dispatcherMgr.dispatchEvent(UserMgr.EVENT_GET_VALUE, this.arrayBonusData[i].type);
                    var endDiamond = startDiamond + Number.parseFloat(addBonus);
                    if (endDiamond <= userMgr.getDiamond()) {
                        this.targetDiamond = endDiamond;
                        var dataEffect = {};
                        dataEffect.type = this.arrayBonusData[i].type;
                        dataEffect.timeDelay = delay + 0.5;
                        dataEffect.end = endDiamond;
                        dispatcherMgr.dispatchEvent(UserMgr.EVENT_EFFECT_LABEL, dataEffect);
                    }
                }
            }
    ...
    getResourceIcon: function () {
        switch (this.type) {
            ...
            case ShopSuccessData.TYPE_TICKET:
                cc.log("GET Resource ticket " + this.idEvent + " " + eventMgr.getOfferTicketImage(this.idEvent));
                return eventMgr.getOfferTicketImage(this.idEvent);
                break;
    ...
    getTitle: function () {
        switch (this.type) {
            ...
            case ShopSuccessData.TYPE_HOUR_VIP:
                ...
                break;
            case ShopSuccessData.TYPE_TICKET:
                return eventMgr.getOfferTicketString(this.idEvent);
                break;
            case ShopSuccessData.TYPE_DIAMOND:
...
var GUIReceiveGift = BaseLayer.extend({
    ...
    pushTicket: function (ticket, posEnd) {
        ...
        if (ticket > 0) {
            ...
            bonusTicket.idEvent = eventMgr.getIdMainEvent();
        }
