var RankMiniGUI = BaseLayer.extend({
    ctor: function () {
        this._super();
        ...
        this.initWithBinaryFile("RankInGameLayer.json");
    },
    ...
    initGUI: function () {
        ...
        this.btnMiniRank = this.customButton("btnMiniRank", RankMiniGUI.BTN_MINI_RANK);
        this.btnMiniRank.setVisible(false);
        this.btnClose = this.customButton("btnClose", RankMiniGUI.BTN_CLOSE);
        this.btnTest = this.customButton("btnTest", RankMiniGUI.BTN_TEST);
        ...
        pTouch.addTouchEventListener(function(layout, type){
            switch(type){
                ...
                case ccui.Widget.TOUCH_CANCELED:
                    this.isHolding = false;
                    this.timeOpenMiniRank = RankMiniGUI.TIME_CLOSE_MINI_RANK * 1000;
                    break;
            }
        }.bind(this), this);
    ...
    onButtonRelease: function (btn, id) {
        switch (id) {
            case RankMiniGUI.BTN_CLOSE: {   //btnClose
                RankData.showMiniRankGUI(false);
                break;
            }
            case RankMiniGUI.BTN_MINI_RANK: {   //btnMiniRank
                if (RankData.checkOpenRank(true)) {
                    RankData.numberOpenGuiRankTracking++;
                    RankData.showMiniRankGUI(true);
                }
                break;
            }
            case RankMiniGUI.BTN_TEST: {   //btnTest
    ...
    showMiniRank: function (isShow) {
        ...
        if (isShow){
            ...
            if (this.myRankIdx != -1) {
                var myRankIdx = RankData.getInstance().getMyRankPosition(true);
    ...
    updateDetailRankInfo: function (forceUpdateIdx) {
        this.data = RankData.getInstance().getDataCurWeek();
        if (!this.data){
            this.data = {};
            this.data.topUser = [];
    ...
    effectRankChange: function(oldIdx, curIdx){
        ...
        var cellHeight = this.myRankInfoCell.bg.getContentSize().height + RankGUI.PADDING_CELL;
    ...
    tableCellAtIndex: function(table, idx){
        ...
        if (!cell){
            cell = new RankPersonalInfoCell(true);
    ...
    tableCellSizeForIndex: function(){
        return cc.size(this.sTableRank.getViewSize().width, this.myRankInfoCell.bg.getContentSize().height + RankGUI.PADDING_CELL);
    },
...
RankMiniGUI.className = "RankMiniGUI";
RankMiniGUI.TIME_CLOSE_MINI_RANK = 3;
RankMiniGUI.BTN_CLOSE = 0;
RankMiniGUI.BTN_MINI_RANK = 1;
RankMiniGUI.BTN_TEST = 2;