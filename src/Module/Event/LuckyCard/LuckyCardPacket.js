...
CmdSendXmasRegisterOffline = CmdSendCommon.extend({

    ctor: function () {
        this._super();
        this.initData(1000);
        this.setControllerId(1);
        this.setCmdId(CMD.VIP_REGISTER_INFO_OFFLINE_EVENT);
    },

    putData: function (uId, name, add, cmnd, phone, email) {
        if (!uId) uId = gamedata.userData.uID;
        if (name === undefined) name = "";
        if (phone === undefined) phone = "";
        if (cmnd === undefined) cmnd = "";
        if (add === undefined) add = "";
        ...
        cc.log("#CmdSendXmasRegisterOffline : " + JSON.stringify(arguments));
