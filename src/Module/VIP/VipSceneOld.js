var VipSceneOld = BaseLayer.extend({
    ...
    onEnterFinish: function () {
        ...
        this.scheduleUpdate();
        VipSceneOld.lastTimeInteract = 0;
        if (!cc.sys.isNative) {
    ...
    initGUI: function () {
        ...
        this.btnClose = this.customButton("btnClose", VipSceneOld.BTN_BACK, this._layout, false);

        this.pTooltip = this.getControl("pTooltip");
        this.pTooltip.defaultPos = this.pTooltip.getPosition();

        this.btnHelp = this.customButton("btnHelp", VipSceneOld.BTN_HELP);
        ...
        this.customButton("btnAvatar", VipSceneOld.BTN_AVATAR, panelUserInfo, false);
        this.customButton("btnShop", VipSceneOld.BTN_GO_SHOP);
        ...
        this.btnTooltip1 = this.customButton("btnTooltip1", VipSceneOld.BTN_TOOLTIP_1, this.iconTime);
        ...
        this.btnTooltip2 = this.customButton("btnTooltip2", VipSceneOld.BTN_TOOLTIP_2, this.bgProgressVip);
        ...
        this.btnTooltip0 = this.customButton("btnTooltip0", VipSceneOld.BTN_TOOLTIP_0, pLevelProcess);
        this.listVipLevelIcons = [];
        for (var i = 1; i <= VipManager.NUMBER_VIP; i++) {
        ...
        this.btnCurrPanel = this.customButton("btnCurrPanel", VipSceneOld.BTN_CURR_PANEL, this.pCurrentBenefit);
        ...
        this.btnPrevious = this.customButton("btnPrevious", VipSceneOld.BTN_PREVIOUS_VIP, this.pNextLevelBenefit);
        this.btnPrevious.imgArrow = this.getControl("imgArrow", this.btnPrevious);
        this.btnNext = this.customButton("btnNext", VipSceneOld.BTN_NEXT_VIP, this.pNextLevelBenefit);
        ...
        this.btnNextPanel = this.customButton("btnNextPanel", VipSceneOld.BTN_NEXT_PANEL, this.pNextLevelBenefit);
        ...
        this.nextListener = cc.EventListener.create({
            ...
            onTouchBegan: function (touch, event) {
                ...
                if (cc.rectContainsPoint(rect, touch.getLocation())) {
                    VipSceneOld.isFollowCurrentLevel = false;
                    VipSceneOld.lastTimeInteract = 0;
                }
                return false;
            },
        ...
        this.btnCheat = this.customButton("btnCheat", VipSceneOld.BTN_CHEAT);
        ...
        this.customButton("btnCheatOld", VipSceneOld.BTN_CHEAT_OLD);
        ...
        this.customButton("btnCheatNew", VipSceneOld.BTN_CHEAT_NEW);
    ...
    onUpdateVipInfo: function () {
        ...
        if (levelVip > 0) {
            ...
            VipSceneOld.addEffectIconVip(this.pDecorate, 1.5, 2, cc.p(0, 0), levelVip);
        }
        ...
        this.iconTime.labelTime.setString(VipManager.getRemainTimeString(VipManager.getInstance().getRemainTime()));

        this.bgProgressVip.setVisible(levelVip < VipManager.NUMBER_VIP);
        ...
        this.lbUserName.setString(userMgr.getDisplayName());
        this.lbGold.setString(StringUtility.pointNumber(userMgr.getGold()));
        this._imgAvatar.asyncExecuteWithUrl(userMgr.getUserName(), userMgr.getAvatar());
        if (this.avatarFrame){
        ...
        VipSceneOld.runEffectProgressVip(this.bgProgressVip, this.progressVip, this.txtExp, this.imgVpoint, 0.7, 0, vpoint, levelVip);
    },
    ...
    addOtherBenefit: function () {
        ...
        for (var i = vipLevel + 1; i <= VipManager.NUMBER_VIP; i++) {
    ...
    changePageOtherBenefit: function (idButton) {
        var isNext = (idButton === VipSceneOld.BTN_NEXT_VIP);
    ...
    onButtonRelease: function (btn, id) {
        var pos;
        switch (id) {
            case VipSceneOld.BTN_BACK: {
                this.onBack();
                break;
            }
            case VipSceneOld.BTN_HELP: {
                sceneMgr.openGUI(NewHelpVipGUI.className, true);
                break;
            }
            case VipSceneOld.BTN_GO_SHOP: {
                paymentMgr.openShop(VipSceneOld.className, true);
                break;
            }
            case VipSceneOld.BTN_PREVIOUS_VIP:
            case VipSceneOld.BTN_NEXT_VIP: {
                this.changePageOtherBenefit(id);
                break;
            }
            case VipSceneOld.BTN_CHEAT: {
                this.pCheat.setVisible(!this.pCheat.isVisible());
                break;
            }
            case VipSceneOld.BTN_CHEAT_OLD: {
            ...
            case VipSceneOld.BTN_CHEAT_NEW: {
            ...
            case VipSceneOld.BTN_TOOLTIP_0: {
            ...
            case VipSceneOld.BTN_TOOLTIP_1: {
            ...
            case VipSceneOld.BTN_TOOLTIP_2: {
    ...
    update: function (dt) {
        ...
        this.iconTime.labelTime.setString(VipManager.getRemainTimeString(remainTime));
        ...
        var target = (VipSceneOld.isFollowCurrentLevel) ? this.currentLevel : this.pageOtherBenefit.getPage(this.currentPageOtherBenefit).detailBenefit;
        var targetCopy = (VipSceneOld.isFollowCurrentLevel) ? this.pageOtherBenefit.getPage(this.currentPageOtherBenefit).detailBenefit : this.currentLevel;
        var positionY = target.getPositionOffestY();
        targetCopy.setPositionOffestY(positionY);

        VipSceneOld.lastTimeInteract += dt;
        if (VipSceneOld.lastTimeInteract > VipSceneOld.TIME_SUGGEST_NEXT_VIP) {
            VipSceneOld.lastTimeInteract = 0;
...
VipSceneOld.className = "VipSceneOld";

VipSceneOld.BTN_BACK = 0;
VipSceneOld.BTN_HELP = 1;
VipSceneOld.BTN_GO_SHOP = 2;
VipSceneOld.BTN_AVATAR = 3;
VipSceneOld.BTN_NEXT_VIP = 4;
VipSceneOld.BTN_PREVIOUS_VIP = 5;
VipSceneOld.BTN_CURR_PANEL = 6;
VipSceneOld.BTN_NEXT_PANEL = 7;
VipSceneOld.BTN_CHEAT_OLD = 8;
VipSceneOld.BTN_CHEAT_NEW = 9;
VipSceneOld.BTN_CHEAT = 10;
VipSceneOld.BTN_TOOLTIP_0 = 11;
VipSceneOld.BTN_TOOLTIP_1 = 12;
VipSceneOld.BTN_TOOLTIP_2 = 13;

VipSceneOld.isFollowCurrentLevel = true;
VipSceneOld.lastTimeInteract = 0;
VipSceneOld.TIME_SUGGEST_NEXT_VIP = 5;

VipSceneOld.runEffectProgressVip = function (bgProgress, progress, txtExp, imgVpoint, timeRun, startVpoint, targetVpoint, currentLevelVip, imgCurVip, imgNextVip) {
    // cc.log("VipSceneOld.runEffectProgressVip: ", startVpoint, targetVpoint, currentLevelVip);
    startVpoint = startVpoint || 0;
    // if (currentLevelVip >= VipManager.NUMBER_VIP){
    ...
    var actions = [];
    var totalVpoint = VipManager.getInstance().getVpointNeed(currentLevelVip);
    ...
    var actionFinal = cc.callFunc(function () {
        ...
        if (targetVpoint >= totalVpoint) {
            ...
            if (newLevel <= VipManager.NUMBER_VIP) {
                var newTotal = VipManager.getInstance().getVpointNeed(newLevel);
                ...
                if (imgCurVip) {
                    ...
                    imgCurVip.loadTexture(VipManager.getIconVip(newLevel));
                    var newLevel2 = newLevel + 1;
                    if (newLevel2 <= VipManager.NUMBER_VIP) {
                        imgNextVip.loadTexture(VipManager.getIconVip(newLevel2));
                        imgNextVip.ignoreContentAdaptWithSize(true);
                    }
                    VipSceneOld.addEffectHighlight(imgNextVip, 0.3, 0.8, cc.p(15, 15));
                    imgNextVip.setVisible(newLevel2 <= VipManager.NUMBER_VIP);
                }
            }
        }
    });
...
VipSceneOld.addEffectHighlight = function (parent, scale, time, position) {
...
VipSceneOld.addEffectIconVip = function (parent, scale, time, position, levelVip) {
    VipSceneOld.addEffectHighlight(parent, scale, time, position);
    ...
    setTimeout(function () {
        ...
        if (effect) {
            ...
            var ngoc = new cc.Sprite(VipManager.getImageUpVip(levelVip));
...
VipSceneOld.scrollToPercentHorizonal = function (tableView, percent, actionTime, isBounceable) {
    ...
    if (isBounceable) {
        actions.push(cc.callFunc(function () {
            VipSceneOld.scrollToPercentHorizonal(tableView, 0, actionTime / 5, false);
        }));
    }
...
var DetailBenefit = BaseLayer.extend({
    ...
    setInfoBenefit: function (level, targetLevel) {
        var texture = VipManager.getImageVip(level);
        if (texture) {
        ...
        this.listBenefit = VipManager.getInstance().getListBenefit(level);
        for (var i = 0; i < this.listBenefit.length; i++) {
            this.listBenefit[i].myBenefit = true;
        }

        var listTemp = VipManager.getInstance().getListBenefit(targetLevel);
    ...
    makeTargetListBenefit: function (listBenefit) {
        ...
        var listFull = VipManager.getInstance().getListBenefit(VipManager.NUMBER_VIP);
        var listFullHave = VipManager.getInstance().getListBenefitHave(VipManager.NUMBER_VIP);
    ...
    setInfoBenefit2: function (oldVipLevel, newVipLevel, oldVipTime, newVipTime, isNewType, goldBonus) {
        if (isNewType) {
            this.iconVip.loadTexture(VipManager.getImageVip(newVipLevel));
        } else {
            if (oldVipLevel >= 0) {
        ...
        for (i = 0; i < this.listBenefit.length; i++) {
            if (this.listBenefit[i]["index"] === "9") {
                ...
                this.listBenefit[i]["value"] = VipManager.getRemainTimeString(time, true);
    ...
    makeFullListBenefit: function (thisLevel, targetLevel, isNewType) {
        var i;
        var listAHave = VipManager.getInstance().getListBenefitHave(thisLevel, !isNewType);
        var listBHave = VipManager.getInstance().getListBenefitHave(targetLevel, isNewType);
        var listA = VipManager.getInstance().getListBenefit(thisLevel, !isNewType);
        var listB = VipManager.getInstance().getListBenefit(targetLevel, isNewType);
    ...
    scrollToEnd: function (actionTime, isBounceable, delayTime) {
        ...
        setTimeout(function () {
            VipSceneOld.scrollToPercentHorizonal(this.viewBenefit, 0, actionTime, isBounceable);
...
var DetailBenefitCell = cc.TableViewCell.extend({
    ...
    setInfo: function (inf, isCurrentLevel, isNewVip) {
        ...
        if (inf.index === ConvertOldVip.INDEX_TIME_REMAIN) { // case dac biet cho gui convert
            ...
            this.iconBenefit.loadTexture(VipManager.getInstance().getImageBenefit(2));
            this.iconLockBenefit.setVisible(false);
        } else {
            this.detail.setString(VipManager.getInstance().getBenefitName(inf.index));
            this.value.setString(VipManager.getInstance().getValueBenefit(inf.index, inf.value, false));
            this.iconLockBenefit.setVisible(VipManager.getInstance().getIsLock(inf.index) && isNewVip && (VipManager.getInstance().getVipLevel() > 0 || !isCurrentLevel));
            this.iconBenefit.loadTexture(VipManager.getInstance().getImageBenefit(inf.index));
        }
        ...
        if (isCurrentLevel) {
            ...
            // cc.log("DetailBenefitCell: ", JSON.stringify(inf), VipManager.getInstance().getValueBenefit(inf.index, inf.value));
        } else {
...
var NewHelpVipGUI = BaseLayer.extend({
    ...
    onEnterFinish: function () {
        ...
        for (var i = 0; i < VipManager.NUMBER_VIP + 1; i++) {
    ...
    initGUI: function () {
        ...
        this.listDetail = [];
        for (i = 0; i < VipManager.NUMBER_VIP + 1; i++) {
        ...
        for (i = 0; i < VipManager.NUMBER_BENEFIT; i++) {
            ...
            title.setString(VipManager.getInstance().getBenefitName(i));
            title.setVisible(i !== 8); // tam an fanpage VIP di
        }

        this.listPanelVip = [];
        for (i = 0; i <= VipManager.NUMBER_VIP; i++) {
    ...
    onUpdateLevel: function () {
        ...
        var listLock = VipManager.getInstance().getListLockTypeBenefit();
        var listBenefitHave = VipManager.getInstance().getListBenefitHave(vipLevel);
...
var BenefitDetailColumn = cc.Layer.extend({
    ctor: function () {
        ...
        this.listTxtType = [];
        for (var i = 0; i < VipManager.NUMBER_BENEFIT; i++) {
            this.listTxtType.push(ccui.Helper.seekWidgetByName(this._layout, "txtType" + i));
        }
    },

    setBenefit: function (level) {
        var listBenefit = VipManager.getInstance().getListBenefit(level, false, true);
        ...
        this.imgVip.loadTexture(VipManager.getIconVip(level));
        ...
        for (var i = 1; i < this.listTxtType.length; i++) {
            ...
            if (benefit) {
                value = (benefit["value"]) ? VipManager.getInstance().getValueBenefit(benefit["index"], benefit["value"], false) : "-";
            } else {
                value = "-"
            }
...
var VipIntroGUI = BaseLayer.extend({
    ...
    onButtonRelease: function (btn, id) {
        if (id > 0) {
            sceneMgr.openScene(VipSceneOldOld.className);
            return;
        }
        this.onClose();
    }
...
var VipChangeGoldSuccess = BaseLayer.extend({
    ...
    setInfoChangeGold: function (data, vpointGet, bonusTime, offerEvent, bonusDiamond) {
        var channelBought = paymentMgr.getShopGoldByType(data.channel);
        ...
        var vipLevel = VipManager.getInstance().getVipLevel();
        var vPointVip1 = VipManager.getInstance().getVpointNeed(0);
        if (vipLevel < 1) {
            hourBonus = 0;
        }
    ...
    getBonusType: function (channelId, packageBought, totalGold, voucherGold) {
        ...
        switch (channelId) {
            case Payment.GOLD_G: {
                promoteType = paymentMgr.getIsFirstGoldG(packageBought["value"]);
                break;
            }
            case Payment.GOLD_IAP: {
                promoteType = paymentMgr.getIsFirstGoldIAP(packageBought["value"]);
                break;
            }
            ...
            case Payment.GOLD_SMS: {
                promoteType = paymentMgr.getIsFirstGoldSMSnew(channelId, packageBought["value"]);
                break;
            }
            case Payment.GOLD_ATM: {
                promoteType = paymentMgr.getIsFirstGoldATM(packageBought["value"]);
                break;
            }
            case Payment.GOLD_ZALO: {
                promoteType = paymentMgr.getIsFirstGoldZalo(packageBought["value"]);
                break;
            }
            case Payment.GOLD_ZING: {
                promoteType = paymentMgr.getIsFirstGoldZing(packageBought["value"]);
                break;
            }
            default: {
        ...
        switch (promoteType) {
            ...
            case Payment.BONUS_VIP:
                ...
                var vipIndex = VipManager.getInstance().getVipLevel();
    ...
    effectChangeGoldSuccess: function () {
        ...
        effectMgr.runLabelPoint(this._uiBean, (userMgr.getGold() - gold), userMgr.getGold(), timeRun);
        ...
        if (diamondBonus > 0) {
            ...
            effectMgr.runLabelPoint(this._uiDiamond, (userMgr.getDiamond() - diamondBonus), userMgr.getDiamond(), timeRun);
        }
        setTimeout(function () {
            var levelVip = VipManager.getInstance().getRealVipLevel();
            var isVip = VipManager.getInstance().isVip();
...
var VipUpLevelGUI = BaseLayer.extend({
    ...
    setChangeBenefit: function (oldLevel, newLevel) {
        ...
        var oldBenefit = VipManager.getInstance().getListBenefit(oldLevel);
        var newBenefit = VipManager.getInstance().getListBenefit(newLevel);
    ...
    changeIconVip: function (oldLevel, newLevel) {
        ...
        var delayTime = 0;
        if (oldLevel > 0 && oldLevel <= VipManager.NUMBER_VIP) {
            var imgOldVip = new ccui.Button(VipManager.getImageVip(oldLevel));
        ...
        setTimeout(function () {
            ...
            var actionBlink = cc.callFunc(function () {
                VipSceneOld.addEffectHighlight(this.pIconVip, 1, 1, cc.p(0, 0));
            }.bind(this));
    ...
    onClose: function () {
        this._super();
        var dailyBonus = VipManager.getInstance().getDailyBonusGold();
        if (dailyBonus) {
    ...
    scrollToEnd: function (actionTime) {
        VipSceneOld.scrollToPercentHorizonal(this.viewBenefitChange, 100, actionTime);
    }
});
...
var BenefitChangeCell = cc.TableViewCell.extend({
    ...
    setInfo: function (inf) {
        ...
        this.benefitDetail.setString(VipManager.getInstance().getBenefitName(inf.index));
        this.imgType.loadTexture(VipManager.getInstance().getImageBenefit(inf.index));
        this.newBenefit.setString(VipManager.getInstance().getValueBenefit(inf.index, inf.value, true));
        this.oldBenefit.setString(VipManager.getInstance().getValueBenefit(inf.index, inf.oldValue, true));
...
var VipDailyGoldBonusGUI = BaseLayer.extend({
    ...
    setInfoDailyBonus: function (gold) {
        ...
        var levelVip = VipManager.getInstance().getRealVipLevel();
        this.imgVip.loadTexture(VipManager.getIconVip(levelVip));
        this.imgVipUser.loadTexture(VipManager.getIconVip(levelVip));
        this.txtVip.setString(StringUtility.replaceAll(localized("VIP_NAME"), "@level", levelVip));
        VipManager.getInstance().saveDailyBonusGold(0);
    },
    ...
    effectCReceiveGold: function () {
        ...
        effectMgr.runLabelPoint(this._uiBean, (userMgr.getGold() - gold), userMgr.getDiamond(), timeRun);
        setTimeout(function () {
            var action = cc.callFunc(function () {
                this.onClose();
            }.bind(this));
...
var ConvertOldVip = BaseLayer.extend({
    ...
    setDataBenefitEqual: function () {
        ...
        var levelVip = VipManager.getLevelWithVpoint(vPointV + vPointG);
    ...
    getActionsByPhase: function (phase) {
        ...
        if (phase === 0) {
            this.iconNextVip.loadTexture(VipManager.getIconVip(1));
            this.progressExp.setPercent(0);
            this.txtExp.setString("0 / " + VipManager.getInstance().getVpointNeed(0));
            ...
            if (vPointV > 0) {
                ...
                strTxt += VipManager.getRemainTimeString(data.oldTime);
                this.txtOldRemainTime.setString(strTxt);
                this.txtVpoint.setString("+ " + vPointV);

                var oldConfig = VipManager.getInstance().getOldVipConfig();
            ...
            var level = VipManager.getLevelWithVpoint(vPointV);
            var vPointRemain = VipManager.getRemainVpoint(vPointV);
            this.iconCurVip.loadTexture(VipManager.getIconVip(level));
            this.iconNextVip.loadTexture(VipManager.getIconVip(level + 1));
            var newVpointNeed = VipManager.getInstance().getVpointNeed(level);
            this.progressExp.setPercent(vPointRemain / newVpointNeed * 100);
            this.txtExp.setString(vPointRemain + " / " + newVpointNeed);
            if (level >= VipManager.NUMBER_VIP) {
            ...
            if (vPointG > 0) {
                actions.push(cc.callFunc(function () {
                    ...
                    var ratio = VipManager.getInstance().getRatioGstarToVpoint();
            ...
            var actionCreateVip = cc.callFunc(function () {
                this.pDecorate.removeAllChildren(true);
                if (levelVip > 0) {
                    VipSceneOld.addEffectIconVip(this.pDecorate, 1, 2, cc.p(0, 0), levelVip);
                }
            }.bind(this));
    ...
    onButtonRelease: function (btn, id) {
        switch (id) {
            ...
            case ConvertOldVip.BTN_CONFIRM: {
                VipManager.getInstance().removeDataConvert();
