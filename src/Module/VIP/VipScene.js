var VipScene = BaseLayer.extend({
    ctor: function () {
        ...
        VipScene.isFollowCurrentLevel = true;
    ...
    initGUI: function () {
        ...
        this.btnClose = this.customButton("btnClose", VipScene.BTN_BACK, this._layout, false);

        this.pTooltip = this.getControl("pTooltip");
        this.pTooltip.defaultPos = this.pTooltip.getPosition();

        this.btnHelp = this.customButton("btnHelp", VipScene.BTN_HELP);
        ...
        this.customButton("btnAvatar", VipScene.BTN_AVATAR, panelUserInfo, false);
        this.customButton("btnShop", VipScene.BTN_GO_SHOP);
        ...
        this.btnTooltip1 = this.customButton("btnTooltip1", VipScene.BTN_TOOLTIP_1, this.iconTime);
        ...
        this.btnTooltip2 = this.customButton("btnTooltip2", VipScene.BTN_TOOLTIP_2, this.bgProgressVip);
        ...
        this.btnTooltip0 = this.customButton("btnTooltip0", VipScene.BTN_TOOLTIP_0, pLevelProcess);
        ...
        this.btnCurrPanel = this.customButton("btnCurrPanel", VipScene.BTN_CURR_PANEL, this.pCurrentBenefit);
        ...
        this.btnPrevious = this.customButton("btnPrevious", VipScene.BTN_PREVIOUS_VIP, this.pNextLevelBenefit);
        this.btnPrevious.imgArrow = this.getControl("imgArrow", this.btnPrevious);
        this.btnNext = this.customButton("btnNext", VipScene.BTN_NEXT_VIP, this.pNextLevelBenefit);
        ...
        this.btnNextPanel = this.customButton("btnNextPanel", VipScene.BTN_NEXT_PANEL, this.pNextLevelBenefit);
        ...
        this.nextListener = cc.EventListener.create({
            ...
            onTouchBegan: function (touch, event) {
                ...
                if (cc.rectContainsPoint(rect, touch.getLocation())) {
                    VipScene.isFollowCurrentLevel = false;
                    VipScene.lastTimeInteract = 0;
                }
                return false;
            },
        ...
        this.btnCheat = this.customButton("btnCheat", VipScene.BTN_CHEAT);
        ...
        this.customButton("btnCheatOld", VipScene.BTN_CHEAT_OLD);
        ...
        this.customButton("btnCheatNew", VipScene.BTN_CHEAT_NEW);
    ...
    onUpdateVipInfo: function () {
        ...
        if (levelVip > 0) {
            ...
            VipScene.addEffectIconVip(this.pDecorate, 1.5, 2, cc.p(0, 0), levelVip);
        }
        ...
        VipScene.runEffectProgressVip(this.bgProgressVip, this.progressVip, this.txtExp, this.imgVpoint, 0.7, 0, vpoint, levelVip);
    },
    ...
    changePageOtherBenefit: function (idButton) {
        var isNext = (idButton === VipScene.BTN_NEXT_VIP);
    ...
    onButtonRelease: function (btn, id) {
        ...
        switch (id) {
            ...
            case VipScene.BTN_GO_SHOP: {
                paymentMgr.openShop(VipScene.className, true);
                break;
            }
            case VipScene.BTN_PREVIOUS_VIP:
            case VipScene.BTN_NEXT_VIP: {
    ...
    update: function (dt) {
        ...
        var target = (VipScene.isFollowCurrentLevel) ? this.currentLevel : this.pageOtherBenefit.getPage(this.currentPageOtherBenefit).detailBenefit;
        var targetCopy = (VipScene.isFollowCurrentLevel) ? this.pageOtherBenefit.getPage(this.currentPageOtherBenefit).detailBenefit : this.currentLevel;
        var positionY = target.getPositionOffestY();
        targetCopy.setPositionOffestY(positionY);

        VipScene.lastTimeInteract += dt;
        if (VipScene.lastTimeInteract > VipScene.TIME_SUGGEST_NEXT_VIP) {
            VipScene.lastTimeInteract = 0;
...
VipScene.BTN_BACK = 0;
VipScene.BTN_HELP = 1;
VipScene.BTN_GO_SHOP = 2;
VipScene.BTN_AVATAR = 3;
VipScene.BTN_NEXT_VIP = 4;
VipScene.BTN_PREVIOUS_VIP = 5;
VipScene.BTN_CURR_PANEL = 6;
VipScene.BTN_NEXT_PANEL = 7;
VipScene.BTN_CHEAT_OLD = 8;
VipScene.BTN_CHEAT_NEW = 9;
VipScene.BTN_CHEAT = 10;
VipScene.BTN_TOOLTIP_0 = 11;
VipScene.BTN_TOOLTIP_1 = 12;
VipScene.BTN_TOOLTIP_2 = 13;

VipScene.isFollowCurrentLevel = true;
VipScene.lastTimeInteract = 0;
VipScene.TIME_SUGGEST_NEXT_VIP = 5;

VipScene.runEffectProgressVip = function (bgProgress, progress, txtExp, imgVpoint, timeRun, startVpoint, targetVpoint, currentLevelVip, imgCurVip, imgNextVip) {
    // cc.log("VipScene.runEffectProgressVip: ", startVpoint, targetVpoint, currentLevelVip);
        ...
        if (targetVpoint >= totalVpoint) {
            ...
            if (newLevel <= VipManager.NUMBER_VIP) {
                ...
                if (imgCurVip) {
                    ...
                    VipScene.addEffectHighlight(imgNextVip, 0.3, 0.8, cc.p(15, 15));
...
VipScene.addEffectHighlight = function (parent, scale, time, position) {
...
VipScene.addEffectIconVip = function (parent, scale, time, position, levelVip) {
    VipScene.addEffectHighlight(parent, scale, time, position);
...
VipScene.scrollToPercentHorizonal = function (tableView, percent, actionTime, isBounceable) {
    ...
    if (isBounceable) {
        actions.push(cc.callFunc(function () {
            VipScene.scrollToPercentHorizonal(tableView, 0, actionTime / 5, false);
        }));
    }
...
var DetailBenefit = BaseLayer.extend({
    ...
    scrollToEnd: function (actionTime, isBounceable, delayTime) {
        ...
        setTimeout(function () {
            VipScene.scrollToPercentHorizonal(this.viewBenefit, 0, actionTime, isBounceable);
...
var VipIntroGUI = BaseLayer.extend({
    ...
    onButtonRelease: function (btn, id) {
        if (id > 0) {
            sceneMgr.openScene(VipScene.className);
            return;
        }
        this.onClose();
    }
...
var VipUpLevelGUI = BaseLayer.extend({
    ...
    changeIconVip: function (oldLevel, newLevel) {
        ...
        setTimeout(function () {
            ...
            var actionBlink = cc.callFunc(function () {
                VipScene.addEffectHighlight(this.pIconVip, 1, 1, cc.p(0, 0));
            }.bind(this));
    ...
    scrollToEnd: function (actionTime) {
        VipScene.scrollToPercentHorizonal(this.viewBenefitChange, 100, actionTime);
    }
});
    ...
    getActionsByPhase: function (phase) {
        ...
        if (phase === 0) {
            ...
            var actionCreateVip = cc.callFunc(function () {
                this.pDecorate.removeAllChildren(true);
                if (levelVip > 0) {
                    VipScene.addEffectIconVip(this.pDecorate, 1, 2, cc.p(0, 0), levelVip);
                }
            }.bind(this));
