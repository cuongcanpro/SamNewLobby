var LobbyScene = BaseLayer.extend({
    ...
    onEnterFinish: function () {
        ...
        // else
        //     this.btnLuckyBonus.setPosition(LuckyBonusManager.LOBBY_BTN_POS_X_WITHOUT_CAMERA_BTN, LuckyBonusManager.LOBBY_BTN_POS_Y);
        LuckyBonusManager.getInstance().checkShowNotify(this.btnLuckyBonus);

        //init fortune cat
        FortuneCatManager.getInstance().checkShowNotify();

        ///remove camera while in fortune cat event
        this.btnCamera.setVisible(false);
    },
    ...
    initCenterLeft: function () {
       ...
       // offerManager.setGroupOffer(this.pOffer);
        ...
        event.initArrayBtnEvent(this.pEvent);
    ...
    initTopRight: function () {
        ...
        ///init lucky bonus and fortune cat; replace btnCamera icon
        ...
        this.btnCamera.loadTextures(
            "res/Lobby/LobbyGUI/btnCameraNew.png",
            "res/Lobby/LobbyGUI/btnCameraNew.png",
            "res/Lobby/LobbyGUI/btnCameraNew.png"
        );
        this.btnCamera.x -= 5;
        ...
        pRightTop.addChild(this.btnFortuneCat);
    },
    ...
    onUpdateGUI: function (data) {
        if (this._uiAvatar && this._uiName && this._uiBean && this._uiCoin) {
            ...
            this._uiBean.setString(StringUtility.formatNumberSymbol(GameData.getInstance().userData.bean));
    ...
    //     }
    //     var that = this;
    //     var manifestPath = "res/Test/projectTest.manifest";
    //     var storagePath = ((jsb.fileUtils ? jsb.fileUtils.getWritablePath() : "/") + "Test/");
        ...
        switch (id) {
            ...
            case LobbyScene.BTN_CHONBAN: {
                if (Config.ENABLE_CHEAT) {
                    //sceneMgr.openGUI(WChallengeCloverProgress.className);
                    //return;
                }
            ...
            case LobbyScene.BTN_WORLD_CUP: {
                decorateManager.openDecorate();
                break;
            }
            ...
            case LobbyScene.BTN_EVENT_IN_GAME: {
                event.openEventInGame(button.idEvent);
                break;
            }
    ...
    updateMinigames: function () {
        return;
        if (Config.DISABLE_FOOTBALL) {
            this._imgMinigameNotice.setVisible(false);
            this._imgMinigameNotice2.setVisible(false);
            return;
        }
        var num = football.fData.listMatch.length;
        if (num > 0) {
            this._imgMinigameNotice.setVisible(true);
            this._lbMinigameAvaiable.setString(num);
            this._imgMinigameNotice2.setVisible(true);
            this._lbMinigameAvaiable2.setString(num);
        } else {
            this._imgMinigameNotice.setVisible(false);
            this._imgMinigameNotice2.setVisible(false);
        }
            ...
            case UserInfoPanel.BTN_SEND_CHAT:
                if (gamedata.gameLogic){
                    for (var i = 0; i < gamedata.gameLogic._players.length; i++){
                        if (gamedata.gameLogic._players[i]._ingame && gamedata.gameLogic._players[i]._info && gamedata.gameLogic._players[i]._info["uID"] == this._user.uID){
...
var GUIMapZalo = BaseLayer.extend({

    ctor: function () {
        this._super(GUIMapZalo.className);
        this.initWithBinaryFile("GUIMapZalo.json");
    },

    initGUI: function () {
        this.bg = this.getControl("bg");
        this.labelName = this.getControl("labelUserName");
        this.labelGameName = this.getControl("labelGameName");
        this.labelName.setString(GameData.getInstance().userData.zName);
        this.labelGameName.setString(localized("GAME_NAME"));
        this.customButton("btnOK", GUIMapZalo.BTN_MAP_ZALO);
        this.enableFog();
    },
...
GUIMapZalo.BTN_MAP_ZALO = 0;
GUIMapZalo.tag = 10;
GUIMapZalo.className = "GUIMapZalo";

var GUIMapZaloSuccess = BaseLayer.extend({

    ctor: function () {
        this._super(GUIMapZaloSuccess.className);
        this.initWithBinaryFile("GUIMapZaloSuccess.json");
    },

    initGUI: function () {
        this.bg = this.getControl("bg");
        this.labelName = this.getControl("labelUserName");
        this.labelAutoMap = this.getControl("labelAutoMap");
        this.labelManualMap = this.getControl("labelManualMap");


        this.customButton("btnOK", GUIMapZaloSuccess.BTN_CONFIRM);

        var bgAvatar = ccui.helper.seekWidgetByName(this._layout, "panelAvatar");
    ...
    setTypeMap: function (type) {
        if (type == GUIMapZaloSuccess.AUTO_MAP) {
            this.labelAutoMap.setVisible(false);
            this.labelManualMap.setVisible(true);
    ...
    setInfoMap: function (username, sessionKey) {
        this.labelName.setString(username);
    ...
    onButtonRelease: function (button, id) {
        switch (id) {
            case GUIMapZaloSuccess.BTN_CONFIRM: {

                GameClient.getInstance().retryManualConnect();
                sceneMgr.addLoading(LocalizedString.to("WAITING")).timeout(15);
                this.onClose();
                break;
            }
...
GUIMapZaloSuccess.BTN_CONFIRM = 0;
GUIMapZaloSuccess.tag = 11;
GUIMapZaloSuccess.AUTO_MAP = 0;
GUIMapZaloSuccess.MANUAL_MAP = 1;
GUIMapZaloSuccess.className = "GUIMapZaloSuccess";

var GUISorry = BaseLayer.extend({
    ctor: function () {
        this._super(GUISorry.className);
        this.initWithBinaryFile("GUISorry.json");
    },

    initGUI: function () {
        this.bg = this.getControl("bg");
        this.labelGold = this.getControl("labelGold");
        this.labelGStar = this.getControl("labelGStar");

        this.customButton("btnClose", GUISorry.BTN_CONFIRM);
        this.enableFog();
    },
    ...
    setInfo: function (gold, gStar) {
        this.labelGold.setString(StringUtility.pointNumber(gold));
        this.labelGStar.setString(StringUtility.pointNumber(gStar));
    },
...
GUISorry.BTN_CONFIRM = 0;
GUISorry.tag = 101;
GUISorry.className = "GUISorry";
