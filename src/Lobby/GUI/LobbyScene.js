var LobbyScene = BaseLayer.extend({
    ...
    onEnterFinish: function () {
        ...
        if (!gamedata.checkInReview()) {
            if (Config.enableMinigame() && Config.ENABLE_VIDEO_POKER) {
                ...
                if (this.btnVideoPoker)
                    this.btnVideoPoker.setVisible(true);
                ...
                if (this.btnVideoPoker)
                    this.btnVideoPoker.setVisible(false);
                this.btnCamera.setVisible(!gamedata.checkDisableSocialViral() && cc.sys.isNative);
            } else if (!Config.enableMinigame() && Config.ENABLE_VIDEO_POKER) {
            ...
            if (this.btnVideoPoker) this.btnVideoPoker.setVisible(false);
            this.btnGiftCode.setVisible(false);
            this.btnVip.setVisible(false);
        ...
        this.schedule(this.updateDelta, 5.0);
        this.scheduleUpdate();
        LobbyButtonManager.getInstance().scheduleUpdate();
        //run Action VideoPoker
        if (this.btnVideoPoker != null && this.btnVideoPoker.isVisible()) {
            this.btnVideoPoker.getChildByName("logo").runEffect();
        }
    ...
    reloadPositionButtonBottom: function () {
        ...
        if (event.isInMainEvent() || event.isEndEvent()) {
    ...
    updateDelta: function () {
        if (Config.ENABLE_ROLLDICE) {
            if (rollDice.isChangeWeek()) {
                var cmd = new CmdSendRollDiceEvent();
                cmd.putData();
...
var UserInfoPanel = BaseLayer.extend({
    ...
    setInfo: function(inf) {
        ...
        this.btnTabInteract.setVisible(this._user.uID != gamedata.userData.uID);
