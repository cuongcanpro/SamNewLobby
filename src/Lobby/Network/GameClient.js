...
var WebsocketClient = cc.Class.extend({
    ...
    connect: function(domain,isSsl)
    {
        ...
        this._port = domain.split(":")[1] ? domain.split(":")[1] : Config.PORT_LIVE_WEB ;
        this._isSsl = isSsl;
        var host = this._host;
        var port = this._port;
...
var GameClient = cc.Class.extend({
    ...
    connect: function () {
        if (Config.ENABLE_CHEAT) {
            ...
            //this._tcpClient.connect("118.102.3.18", 10126);
            ...
            if (cc.sys.isNative) {
                if (cc.sys.localStorage.getItem("ipapp")) {
                    //    this._tcpClient.connect("118.102.3.11", 80);
                ...
                if (cc.sys.localStorage.getItem("ssl")) {
                    ...
                    cc.log("_________ CONNECT SERVER LIVE WEB CONFIG : " + Config.SERVER_LIVE + ":" + Config.PORT_LIVE);
                }
            }
        }
...
GameClient.destroyInstance = function () {
    ...
    if (Config.ENABLE_CHEAT && cc.sys.os == cc.sys.OS_IOS) {
        // bi loi crash tren IOS
    }
    else {
        if (cc.sys.isNative){
