var PotBreaker = cc.Class.extend({
    ...
    effectEventButton: function () {
        ...
        this.buttonLobby.time.setFontSize(14);
        ...
        if (btnAnim) {
            ...
            this.buttonLobby.anim.setScale(0.8);
            btnAnim.setPosition(4, 20);
        ...
        if (this.isFinishDownload) {
            if (this.getTimeLeft() > 0) {
                ...
                this.buttonLobby.time.setPositionX(this.buttonLobby.time.pos.x + 10);
            }
        }
        else {
    ...
    addAccumulateGUI: function(){
        ...
        if (!gui){
            gui = sceneMgr.openGUI(PotBreakerAccumulateGUI.className, PotBreaker.GUI_ACCUMULATE, PotBreaker.GUI_ACCUMULATE);
            gui.setVisible(true);
        }
    },
    ...
    onAccumulate: function (cmd) {
        ...
        if (CommonLogic.checkInBoard()) {
            var gui = sceneMgr.openGUI(PotBreakerAccumulateGUI.className, PotBreaker.GUI_ACCUMULATE, PotBreaker.GUI_ACCUMULATE);
            if (gui) {
