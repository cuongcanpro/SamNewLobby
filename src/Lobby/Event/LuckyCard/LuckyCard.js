...
var LuckyCard = cc.Class.extend({
    ...
    isInGUIEvent: function () {
        ...
        return !!(gui && gui instanceof LuckyCardScene);
    },
    ...
    showNotifyEvent: function (btn) {
        ...
        if (this.isInEvent()) {
            ...
            //if(this.buttonLobby && this.buttonLobby.notify) this.buttonLobby.notify.setVisible(this.notifyEvent);
            if (this.isFinishDownload) {
                if (this.checkNewDay()) {
                    var gui = sceneMgr.getMainLayer();
    ...
    showEventButton: function () {
        cc.log("SHOW EVENT BUTTON ********************8 " + this.isEndEvent() + " " + this.isInEvent());
        ...
        if (this.isInEvent() && this.getTimeLeft() > 0) {
            ...
            if (this.buttonLobby.isVisible()) {
                // cc.log("MidAutumn::showEventButton --> Effect");
                cc.log("SHOW EVENT BUTTON ********************3 ");
                this.effectEventButton();
                this.buttonLobby.setVisible(true);
            } else {
                // cc.log("MidAutumn::showEventButton --> Animation");
                cc.log("SHOW EVENT BUTTON ********************4 ");
                this.buttonLobby.setVisible(true);
                this.buttonLobby.setScale(0);
                this.buttonLobby.runAction(cc.spawn(new cc.EaseBackOut(cc.scaleTo(0.5, 1.1)), cc.callFunc(this.effectEventButton.bind(this))));
            }
            //this.buttonLobby.notify.setVisible(this.notifyEvent);
        } else {
            cc.log("SHOW EVENT BUTTON ********************5 " + this.getTimeLeft());
            this.buttonLobby.setVisible(false);
        }
    },

    effectEventButton: function () {
        cc.log("CHAY VO DAY NE *** 1");
        if (!this.buttonLobby) return;
        cc.log("CHAY VO DAY NE *** 2");
    ...
    addAccumulateGUI: function () {
        ...
        if (!gui) {
            var gui = sceneMgr.openGUI(LuckyCardAccumulateGUI.className, LuckyCard.GUI_ACCUMULATE, LuckyCard.GUI_ACCUMULATE);
            gui.setAllShow(true);
        }
    },
    ...
    isInEvent: function () {
        return this.eventTime >= LuckyCard.WEEK_1 && this.eventTime <= LuckyCard.WEEK_2;
    },

    isEndEvent: function () {
        return this.eventTime == LuckyCard.WEEK_3;
    },
    ...
    onEvent: function (cmd) {
        ...
        //        gui.onBack();
        //        setTimeout(function(){
        //            sceneMgr.openGUI(LuckyCardNotifyEndEventGUI.className);  //hien thi GUI thong bao ket thuc Event
        //        }, 500);
        //    }
        //    else
        //        gui.updateEventInfo();
    ...
    updateEventLoop: function (dt) {
        ...
        if (nTime <= 0) {
            ...
            var s = LocalizedString.to("LUCKYCARD_INFO_TIME_LEFT_0");
            if (this.isFinishDownload) {
    ...
    onReceive: function (cmd, data) {
        //if(!Event.ENABLE_LUCKY_CARD) return;
