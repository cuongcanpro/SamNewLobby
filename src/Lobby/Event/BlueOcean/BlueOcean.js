...
BlueOcean = cc.Class.extend({
    ...
    checkNotifyBonusGPanel: function () {
        ...
        return curTime - lastTime > BO_NOTIFY_BONUS_G_COUNT_DOWN;
    },
    ...
    isInEvent: function () {
        return this.eventTime >= BO_WEEK_START && this.eventTime <= BO_WEEK_END;
    },

    isEndEvent: function () {
        return this.eventTime == BO_WEEK_END + 1;
    },
    ...
    onEvent: function (cmd) {
        ...
        // Do Bat dau Event User da co Levl Ruong la 1, nen moi Logic se lay Level cua ruong tru di 1
        this.arrayBonusData = [];
    ...
    onAccumulate: function (cmd) {
        if (!this.isFinishDownload)
            return;
        var gui = sceneMgr.openGUI(BO_ACCUMULATE_GUI_CLASS, BO_ACCUMULATE_GUI_ORDER, BO_ACCUMULATE_GUI_ORDER);
