...
MidAutumn = cc.Class.extend({
    ...
    effectEventButton: function () {
        try {
            ...
            this.buttonLobby.button.setContentSize(120, 120);
    ...
    checkNotifyBonusGPanel: function () {
        ...
        return curTime - lastTime > MD_NOTIFY_BONUS_G_COUNT_DOWN;
    },
    ...
    isInEvent: function () {
        return this.eventTime >= MD_WEEK_START && this.eventTime <= MD_WEEK_END;
    },

    isEndEvent: function () {
        return this.eventTime == MD_WEEK_END + 1;
    },
    ...
    onAccumulate: function (cmd) {
        var gui = sceneMgr.openGUI(MD_ACCUMULATE_GUI_CLASS, MD_ACCUMULATE_GUI_ORDER, MD_ACCUMULATE_GUI_ORDER);
            ...
            for (var s in cmd.histories) {
                ...
                for (var xxx in obj["gifts"]) {
                    ...
                    if (midAutumn.isLamp(ggg["id"])) {
                        ...
                        rObj.isNews[ggg["id"]] = obj["rollGiftIds"].indexOf(ggg["id"]) < 0;
                    }
                        ...
                        for (var idx = 0; idx < ggg["number"].length; idx++) {
                            if (ggg["number"][idx] > 0) {
                                ...
                                rObj.isNews[gId] = obj["rollGiftIds"].indexOf(gId) < 0;
                            }
                        }
                    } else {
        ...
        for (var i = 0; i < cmd.rolledId.length; i++) {
            if (cmd.rolledNumber[i] > 0) {
                ...
                if (Array.isArray(obj)) {
                    lampModel.isNew = obj.indexOf(lampModel.id) < 0;
                }
                else {
