...
var CmdSendEventTetGetInfoTop = CmdSendCommon.extend({
    ctor: function () {
        this._super();
        this.initData(100);
        this.setControllerId(1);
        this.setCmdId(EventTet.CMD_TOP_INFO);
    },

    putData: function (week) {
        cc.log("PACKAGE GET TOP ***** " + week);
        this.packHeader();
        this.putByte(week);
        this.updateSize();
    }
...
var CmdSendEventTetGetMyRank = CmdSendCommon.extend({
    ctor: function () {
        this._super();
        this.initData(100);
        this.setControllerId(1);
        this.setCmdId(EventTet.CMD_MY_RANK);
    },

    putData: function (week) {
        cc.log("CmdSendEventTetGetMyRank: " + week);
        this.packHeader();
        this.putByte(week);
        this.updateSize();
    }
...
CmdSendEventTetCheatToken = CmdSendCommon.extend({
    ctor: function () {
        this._super();
        this.initData(100);
        this.setControllerId(1);
        this.setCmdId(EventTet.CMD_CHEAT_TOKEN);
    },

    putData: function (userId, num) {
        this.packHeader();
        cc.log("USER ID " + userId);
        this.putInt(userId);
        this.putShort(num.length);
...
CmdReceiveEventTetMapLixi = CmdReceivedCommon.extend({
    ...
    readData: function () {
        ...
        for (var i = 0; i < this.mapLixi.length; i++) {
            this.arrayData[i] = new LixiData();
            if (this.mapLixi[i].indexOf(EventTet.TYPE_GOLD) >= 0) {
                this.arrayData[i].type = EventTet.TYPE_GOLD;
            }
            else if (this.mapLixi[i].indexOf(EventTet.TYPE_DIAMOND) >= 0) {
                this.arrayData[i].type = EventTet.TYPE_DIAMOND;
            }
            else {
                this.arrayData[i].type = EventTet.TYPE_TOKEN;
            }
            if (this.mapLixi[i].indexOf("+") >= 0) {
                this.arrayData[i].state = LixiData.OPEN;
                var arr = this.mapLixi[i].split("+");
                this.arrayData[i].value = parseInt(arr[1]);
            }
            else {
                this.arrayData[i].state = LixiData.CLOSE;
                var arr = this.mapLixi[i].split("-");
                this.arrayData[i].value = parseInt(arr[1]);
            }
        }
        this.isOpenAllLixi = this.getByte();
...
var CmdReceiveEventTetTopInfo = CmdReceivedCommon.extend({
    ctor: function (pkg) {
        this._super(pkg);
        this.readData();
    },
    ...
    readData: function () {
        ...
        this.topTokens = this.getLongs();
        this.topAward = this.getInts();
    }
});

var CmdReceiveEventTetMyRankInfo = CmdReceivedCommon.extend({
    ctor: function (pkg) {
        this._super(pkg);
        this.readData();
    },
