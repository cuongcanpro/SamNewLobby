...
let FortuneCatMainLayer = BaseLayer.extend({
    ctor: function(){
    ...
    animateTooltip: function(){
        ...
        let fingerAnimationExpectedDuration = 2000;
        setTimeout(function() {
            this.finger.setVisible(false);
            this.fingerTrait.setVisible(false);
    ...
    animateFinger: function(dt){
        ...
        let numLoopFinger = 50;
        ///trait revealed in 33 loops, each loop reveal 1/33 the trait
        let numLoopTrait = 32;

        ///animate finger
        let deltaY = FortuneCatMainLayer.FINGER_MOVE_DISTANCE_Y / numLoopFinger;
        ///some maths
        let distanceY = Math.abs(this.finger.y - FortuneCatMainLayer.FINGER_START_Y);
        let distanceX = -9 * Math.pow(distanceY, 2) / 25 + 69 * distanceY / 5;
    ...
    addCatSlotRect: function(){
        let catSlotWidth = 86;
        let catSlotHeight = 86;
        let slotX = 0;
        let slot1Y = 227;
        let slot2Y = 132;
        let slot3Y = 37;

        let rect1 = cc.rect(slotX, slot1Y, catSlotWidth, catSlotHeight);
        let rect2 = cc.rect(slotX, slot2Y, catSlotWidth, catSlotHeight);
        let rect3 = cc.rect(slotX, slot3Y, catSlotWidth, catSlotHeight);
    ...
    updateCatSlotList: function(userCatIdList){
        ...
        for (let i = 0; i < FortuneCatManager.getInstance().maxCat; i++){
            let slot = this.getControl("slot_" + (i + 1).toString(), this.catSlots);
            let cat = this.getControl("cat", slot);
            let bgTime = this.getControl("bgTime", slot);
            let time = this.getControl("time", slot);
            let silhouette = this.getControl("silhouette", slot);
            ...
            if (typeof userCatIdList[i] !== "undefined"){
                ...
                let timeValue = FortuneCatUtility.formatCatTime(FortuneCatManager.getInstance().catConfigList[userCatIdList[i]].openTime / 1000);
    ...
    onTouchCatBegan: function(touch, event){
        ...
        this.unschedule(this.animateFinger);
        let touchLocation = this.catSlots.convertToNodeSpace(touch.getLocation());

        for (let i = 0; i < this.catSlotRects.length; i++){
            if (cc.rectContainsPoint(this.catSlotRects[i], touchLocation) &&
    ...
    onTouchCatEnded: function(touch, event){
        let touchLocation = this.mainCat.convertToNodeSpace(touch.getLocation());
        ///open cat
    ...
    onTouchProgressTooltipBegan: function(touch, event){
        let touchLocation = this.progressTooltip.convertToNodeSpace(touch.getLocation());
    ...
    updateUnlockingCatInfo: function(catId, catIndex){
        let unlockingCatIndex = catIndex;
        ...
        let catTitle = this.getControl("title", this.mainCatBody);
        let gold = this.getControl("gold", this.mainCatBody);

        let currentCat = FortuneCatManager.getInstance().catConfigList[catId];
        let goldValue = FortuneCatUtility.formatCatGold(currentCat.gold);
    ...
    updateClockView: function(duration){
        let clock = this.getControl("clock", this.mainCatBody);
        let time = FortuneCatUtility.formatTime(duration);
    ...
    loadUnlockingCatImage: function(catId){
        ...
        if (typeof sp !== 'undefined') {
            let unlockingCatImageX = 151;
            let unlockingCatImageY = 50;
    ...
    updateCatTitle: function(catId){
        let title = this.getControl("title", this.mainCatBody);
    ...
    onButtonRelease: function(btn, id){
        switch (id){
            ...
            case FortuneCatMainLayer.BTN_CHEAT_BELL:
                let numBell = this.numBellCheat.getString();
            ...
            case FortuneCatMainLayer.BTN_CHEAT_CAT:
                let input = this.catIdListCheat.getString();

                input = input || "";
                let catIdList = input.split(",");
...
///animation config
FortuneCatMainLayer.AFK_THRESHOLD = 3;
