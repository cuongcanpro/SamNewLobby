...
var EventData = cc.Class.extend({
    ...
    onEventShopBonusNew: function (data) {
        ...
        event.promoTicket = data.promoTicket;
        // event.promoTicket = 200;
        event.startPromoTicket = data.startPromoTicket;
        event.endPromoTicket = data.endPromoTicket;
        if (this.isMain) {
...
var Event = cc.Class.extend({
    ...
    ctor: function () {
        ...
        this.arrayEvent = [];
        this.arrayBtnEvent = [];
        // this.arrayEvent.push(this.createEvent(Event.WEEKLY_CHALLENGE, Event.WEEKLY_CHALLENGE_NAME, false, WChallenge.getInstance(), false, true, 3));
        // this.arrayEvent.push(this.createEvent(Event.LUCKY_CARD, Event.LUCKY_CARD_NAME, false, luckyCard, false, true, 1));
        // this.arrayEvent.push(this.createEvent(Event.POT_BREAKER, Event.POT_BREAKER_NAME, false, potBreaker, false, true, 3));
        // this.arrayEvent.push(this.createEvent(Event.BLUE_OCEAN, Event.BLUE_OCEAN_NAME, true, blueOcean, false, true, 1));
        // this.arrayEvent.push(this.createEvent(Event.MID_AUTUMN, Event.MID_AUTUMN_NAME, true, midAutumn, false, true, 1));

        // this.arrayEvent.push(this.createEvent(Event.EGG_BREAKER, Event.EGG_BREAKER_NAME, true, eggBreaker, false, true, 3));

        // this.arrayEvent.push(new EventData(Event.MID_AUTUMN, true));
        this.arrayEvent.push(this.createEvent(Event.EVENT_TET, Event.EVENT_TET_NAME, true, eventTet, true, true, 1));
    ...
    showButtonEvent: function (btn) {
        if (this.btnMainEvent) {
            ...
            this.btnMainEvent = new EventButton();
    ...
    openEvent: function (id) {
        ...
        // hien tai thi cach mo Event chinh va phu nhu nhau
    ...
    isInEvent: function (id) {
        var event = this.getEventById(id);
        if (event && event.isInEvent && event.dataEvent.isActive) {
    ...
    isEndEvent: function (id) {
        var event = this.getEventById(id);
        if (event && event.isEndEvent && event.dataEvent.isActive) {
    ...
    // check dang co Main Event hay khong (co the da ket thuc)
    ...
    // get Mang config Shop Event theo loai Payment (dung cho mua ve truc tiep)
    getDataPaymentById: function (id) {
        var event = this.getEventById();
        if (event)
    ...
    // tra ve mang config Ticket cua Event chinh dang chay (thuong la Event Out Game, ban ve truc tiep)
    ...
    notifyEventRegister: function (cmd) {
        ...
        // lay id thuc su cua Event: eggbreaker#date
...
var EventButton = cc.Node.extend({
    ctor: function (isMainEvent) {
        ...
        // hinh anh mac dinh cho Event, se duoc hien thi neu khong co animation
        ...
        // node animation, de them effect vao button Event, tung Event se cai dat rieng
    ...
    touchEvent: function () {
        event.openEventInGame(this.dataEvent.idEvent);
    },

    setInfo: function (dataEvent) {
        this.dataEvent = dataEvent;
        // Do Event WChallenge an het cac thanh phan cua Button Event, phai show lai
