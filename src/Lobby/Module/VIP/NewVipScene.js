var NewVipScene = BaseLayer.extend({
    ...
    onUpdateVipInfo: function () {
        ...
        this.setLabelText(gamedata.getDisplayName(), this.lbUserName);
...
var VipChangeGoldSuccess = BaseLayer.extend({
    ...
    setInfoChangeGold: function (data, vpointGet, bonusTime, offerEvent, bonusDiamond) {
        ...
        if (offerEvent && offerEvent.length > 0) {
            ...
            var iconWidth = this.imgTicket.getContentSize().width * this.imgTicket.getScale();
    ...
    effectChangeGoldSuccess: function () {
        ...
        if (mainLayer instanceof LobbyScene || mainLayer instanceof ShopIapScene) {
            ...
            mainLayer.setGold(startGold + Number.parseFloat(gold), mainLayer.getDiamond());
        }
        else {
            startGold = gamedata.userData.bean - gold;
        }
        endGold = startGold + Number.parseFloat(gold);
        cc.log("START GOLD " + startGold + " END GOLD " + endGold);
        effectMgr.runLabelPoint(this._uiBean, startGold, endGold, timeRun);
