...
var ChatMgr = cc.Class.extend({
    ...
    addChatUser: function(sender, toId, message){
        ...
        if (CheckLogic.checkInBoard() && gamedata.gameLogic){
            for (var i = 0; i < gamedata.gameLogic.players.length; i++){
                if (gamedata.gameLogic.players[i].ingame && gamedata.gameLogic.players[i].info != null && sender.uId == gamedata.gameLogic.players[i].info["uID"]) {
    ...
    onReceiveChatRoom: function(pk){
        ...
        if (gamedata.gameLogic){
            ...
            var username = "";
            for (var i = 0; i < gamedata.gameLogic.players.length; i++){
                if (gamedata.gameLogic.players[i].ingame && gamedata.gameLogic.players[i].chairInServer != null && pk.playerId == gamedata.gameLogic.players[i].chairInServer) {
                    ChatMgr.playChatEffect(i, pk.message);
                    uId = gamedata.gameLogic.players[i].info["uID"];
                    username = gamedata.gameLogic.players[i].info["uName"];
                    break;
                }
            }
...
ChatMgr.playChatEffect = function(index, message){
    ...
    switch(index){
        case 0: //self
            ...
            chatBg.setPosition(-Math.min(chatBg.getContentSize().width, 15), arrow.getContentSize().height * arrow.getScale());
