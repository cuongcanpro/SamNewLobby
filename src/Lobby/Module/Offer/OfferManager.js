...
var OfferManager = cc.Class.extend({
    ...
    addOfferToGroup: function () {
        this.reloadOffer();
        cc.log("addOfferToGroup " + this.arrayOffer.length);
        if (this.haveOffer()) {
    ...
    removeOfferNotAvailable: function () {
        for (var i = 0; i < this.arrayOffer.length; i++) {
            if (this.arrayOffer[i].offerId < 0) {
                cc.log("REMOVE OFFER NOT AVAILABLE " + this.arrayOffer[i].offerId);
    ...
    onReceived: function (cmd, data) {
        ...
        switch (cmd) {
            ...
            case OfferManager.CMD_OFFER_SUCCESS:
                ...
                var convertType = cmd.channel;
              //  cc.log("CONVERT TYPE " + convertType);
                //if (convertType == Payment.GOLD_SMS)
                   // convertType = Payment.GOLD_SMS_VIETTEL;
               // cc.log("CONVERT TYPE ** " + convertType);
